...
for( c = arg3; *c; ++c) {
  ...
  else if( !Q_stricmp( arg1, "draw" ) )
  {
		...
		{
		  trap_SendServerCommand( ent - g_entities, va( "print \"callvote: "
        "draw: Failed, need Sudden Death first\n\"") );
		  return;
		}
		else
		{
   ...
   {
	  if( g_humanStage.integer != S3 || g_alienStage.integer != S3 )
	  {
		  trap_SendServerCommand( ent - g_entities, va( "print \"callvote: "
        "^1Failed, both teams must have S3, To call Sudden Death\n\"") );
		  return;
	  }
  ...
  trap_SendServerCommand( -1, va( "print \"%s" S_COLOR_WHITE
         " called a vote: %s\n\"", ent->client->pers.netname, level.voteDisplayString ) );
           trap_SendServerCommand( -1, "cp \"^1VOTE\n\"" );
...
void Cmd_CallTeamVote_f( gentity_t *ent )
{
  ...
  for( i = 0 ; i < level.maxclients ; i++ )
  {
    if( level.clients[ i ].pers.connected == CON_DISCONNECTED )
      continue;
     trap_SendServerCommand( i, "cp \"^1VOTE \n\"");
