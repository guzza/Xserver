...
qboolean G_issd( gentity_t *ent, int skiparg )
{
	if( g_suddenDeath.integer )
	{
        ...
        "print \"^1Sudden Death is ON\n\"" ) );
		return qtrue;
	}
	else
	{
		trap_SendServerCommand( ent-g_entities, va(
        "print \"^1Sudden Death is OFF\n\"" ) );
		return qtrue;
	}
}
...
qboolean G_admin_namelog( gentity_t *ent, int skiparg )
{
  ...
  for( i = 0; i < MAX_ADMIN_NAMELOGS && g_admin_namelog[ i ]; i++ )
  {
    ...
    if(!Q_strncmp( g_admin_namelog[ i ]->ip, g_admin_bans[ count ]->ip, ip_len))
    {ADMBP( "^7" );break;}
    else if(!Q_strncmp( g_admin_namelog[ i ]->ip, g_admin_bans[ count ]->ip, ip_len2))
    {ADMBP( "^3" );break;}
    else if(!Q_strncmp( g_admin_namelog[ i ]->ip, g_admin_bans[ count ]->ip, ip_len3))
    {ADMBP( "^1" );break;}
     }
    ADMBP( va( "%-2s (*%s) %15s^7",
      (g_admin_namelog[ i ]->slot > -1 ) ?
...
qboolean G_admin_namelog( gentity_t *ent, int skiparg )
{
  ...
  for( i = 0; i < MAX_ADMIN_NAMELOGS && g_admin_namelog[ i ]; i++ )
  {
    ...
    {ADMBP( "^1" );count = MAX_ADMIN_BANS;}
    else if(!Q_strncmp( g_admin_namelog[ i ]->ip, g_admin_bans[ count ]->ip, ip_len2))
    {ADMBP( "^3" );count = MAX_ADMIN_BANS;}
    else if(!Q_strncmp( g_admin_namelog[ i ]->ip, g_admin_bans[ count ]->ip, ip_len))
    {ADMBP( "^7" );count = MAX_ADMIN_BANS;}
     }
    ADMBP( va( "%-2s (*%s) %15s^7",
      (g_admin_namelog[ i ]->slot > -1 ) ?
        va( "%d", g_admin_namelog[ i ]->slot ) : "-",
